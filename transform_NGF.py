...
def map_all_images_using_NGF(I_dir, J_dir, save_dir, thermal_texture_dir, date, params={}, thermal_ext='tiff', dof='affine', eval=False):
    ...
    for k in tqdm(range(batch_size)):
        pyramid_I_temp = list(pyramid_gaussian(I_imgs[k], downscale=downscale)) # multichannel=False removed
        pyramid_J_temp = list(pyramid_gaussian(J_imgs[k], downscale=downscale)) # multichannel=False removed
